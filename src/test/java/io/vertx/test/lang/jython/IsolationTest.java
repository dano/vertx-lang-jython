...
public class IsolationTest extends VertxTestBase {
  ...
  public void testDifferentVerticleTypeDontShareTheSameGlobal() {
    vertx.deployVerticle("verticle_sharing_global_1.py", ar1 -> {
      assertTrue(ar1.succeeded());
      assertEquals(1, callbacks.size());
      assertEquals(1, callbacks.get(0).get().intValue());
      vertx.deployVerticle("verticle_sharing_global_2.py", ar2 -> {
  ...
  public void testSameVerticleTypeShareTheSameGlobal() {
    vertx.deployVerticle("verticle_incrementing_global.py", ar1 -> {
      assertTrue(ar1.succeeded());
      assertEquals(1, callbacks.size());
      assertEquals(1, callbacks.get(0).get().intValue());
      vertx.deployVerticle("verticle_incrementing_global.py", ar2 -> {
        assertTrue(ar2.succeeded());
        assertEquals(2, callbacks.size());
  ...
  public void testSameVerticleTypeDontShareTheSameCvar() {
    vertx.deployVerticle("verticle_incrementing_cvar.py", ar1 -> {
      assertTrue(ar1.succeeded());
      assertEquals(1, callbacks.size());
      assertEquals(1, callbacks.get(0).get().intValue());
      vertx.deployVerticle("verticle_incrementing_cvar.py", ar2 -> {
        assertTrue(ar2.succeeded());
        assertEquals(2, callbacks.size());
        assertEquals(1, callbacks.get(0).get().intValue());
        assertEquals(1, callbacks.get(1).get().intValue());
